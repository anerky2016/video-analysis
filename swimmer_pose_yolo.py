...
class SwimmerPoseYOLO:
    def __init__(self,
        ...
        """
        Initialize YOLO-Pose based swimmer pose detector.
    ...
    def draw_skeleton(self, image: np.ndarray, keypoints: np.ndarray,
        ...
        # Draw connections with bold lines
        for connection in self.filtered_skeleton:
            ...
            if (keypoint_scores[start_idx] > self.detection_threshold and
                ...
                # Use bold line (thickness 5) with bright green color
        ...
        for idx in enabled_indices:
            if keypoint_scores[idx] > self.detection_threshold:
                ...
                # Draw larger keypoint circles (radius 6 with white border)
                cv2.circle(img, point, 6, (255, 255, 255), 2)  # White border
                cv2.circle(img, point, 4, (0, 0, 255), -1)     # Red fill
    ...
    def process_frame(self, frame: np.ndarray,
        ...
        if len(results) > 0 and results[0].keypoints is not None:
            ...
            if keypoints_data.xy.shape[0] > 0:  # Check if any person detected
                # Get the first person detected
                keypoints = keypoints_data.xy[0].cpu().numpy()  # (17, 2)
                keypoint_scores = keypoints_data.conf[0].cpu().numpy()  # (17,)
    ...
    def process_video(self, video_path: str, output_path: Optional[str] = None):
        ...
        while cap.isOpened():
            ...
            cv2.imshow('Swimmer Pose Analysis (YOLOv8-Pose)', processed_frame)
