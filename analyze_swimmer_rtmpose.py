#!/usr/bin/env python3
import argparse
from swimmer_pose_rtmpose import SwimmerPoseRTMPose


def main():
    parser = argparse.ArgumentParser(description='Analyze swimmer posture from video using RTMPose')
    parser.add_argument('input_video', type=str, help='Path to input video file')
    parser.add_argument('-o', '--output', type=str, default=None,
                       help='Path to save output video (optional)')
    parser.add_argument('-m', '--model', type=str, default='rtmpose-l',
                       choices=['rtmpose-l', 'rtmpose-m', 'rtmpose-s'],
                       help='RTMPose model size (l=large, m=medium, s=small). Default: rtmpose-l')
    ...
    print("Initializing RTMPose swimmer pose detector...")
    print(f"Model: {args.model}")
    print(f"Device: {args.device}")

    detector = SwimmerPoseRTMPose(
